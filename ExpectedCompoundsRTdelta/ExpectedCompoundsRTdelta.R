...
if (!requireNamespace("rjson", quietly = TRUE)) {
  # If not installed, install 'rjson'
...
## Define variables to store the parent compound's data
parentRT <- NULL
parentID <- NULL
...
for (ci in 1:nrow(CD.input.ExpCmp)) {
  ...
  if (is.na(compChange) | compChange == "") {
    ...
    parentRT <- as.numeric(CD.input.ExpCmp[ci, "RT in min"])
    parentID <- CD.input.ExpCmp[ci, "Expected Compounds ID"]
    break  # Break the loop after finding the parent compound
  }
}

# create result vector: rt difference from Parent compound
...
for (ci in 1:nrow(CD.input.ExpCmp)) {
  ...
  currRT <- as.numeric(CD.input.ExpCmp[ci, "RT in min"])
...
newcolumn <-list()
newcolumn[[1]] = "RT diff PC"   ## columnName
newcolumn[[2]] = ""     ## IsID
newcolumn[[3]] = "Float"      ## DataType
newcolumn[[4]] <- list(PositionAfter="RT in min")    ## Options
